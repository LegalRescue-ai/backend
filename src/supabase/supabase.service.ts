  ...
  async insertData(data: any, clientIp: string) {
    ...
    try {
      if (!this.emailId && this.collectedData.email) {
        ...
        console.log(
          'Email and IP address inserted successfully with ID:',
          this.emailId,
        );
      ...
      if (this.emailId && this.collectedData.selectedMembership) {
        console.log(
          'Attempting to update record with collected data and IP address...',
        );

        // Fetch the maximum waitlist position, so we can update the next available position
        const { data: maxPositionData, error: maxPositionError } =
          await this.supabase
            .from('waitlist')
        ...
        console.log(
          'Record updated successfully with IP address:',
          updatedData,
        );

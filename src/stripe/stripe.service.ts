...
export class StripeService {
    ...
    try {
      ...
      if (customers.data.length > 0) {
        ...
        const activeSubscriptions =
          await this.checkExistingSubscriptions(customerId);

        if (activeSubscriptions) {
          this.logger.debug(
            'Customer has active subscriptions, creating billing portal session',
          );
          session = await this.createBillingPortalSession(customerId);
        } else {
          this.logger.debug(
            'Customer has no active subscriptions, creating new checkout session',
          );
  ...
  private calculateInitialPrice(
    ...
    if (discountTier.secondYearDiscount > 0) {
      return Math.round(
        basePrice * (1 - discountTier.secondYearDiscount / 100),
      );
    }

    if (discountTier.additionalDiscount) {
      return Math.round(
        basePrice * (1 - discountTier.additionalDiscount.percent / 100),
      );
    }
  ...
  private async createInitialCheckoutSession(
    ...
    try {
      ...
      const initialPriceId = await this.createOrGetPrice(
        initialPrice,
        'discount',
      );
  ...
  private async createSubscriptionSchedule(
    ...
    try {
      this.logger.debug('Starting createSubscriptionSchedule with params:', {
        ...
        basePrice,
        discountTier:
          typeof discountTier === 'string'
            ? discountTier
            : JSON.stringify(discountTier),
      });
      ...
      if (!parsedDiscountTier) {
        this.logger.debug(
          'No discount tier - updating subscription to full price',
        );
      ...
      if (parsedDiscountTier.secondYearDiscount > 0) {
        const discountedAmount = Math.round(
          basePrice * (1 - parsedDiscountTier.secondYearDiscount / 100),
        );
        const discountedPriceId = await this.createOrGetPrice(
          discountedAmount,
          'discount',
        );
        ...
        const additionalDiscountAmount = Math.round(
          basePrice * (1 - parsedDiscountTier.additionalDiscount.percent / 100),
        );
        const additionalDiscountPriceId = await this.createOrGetPrice(
      ...
      this.logger.debug(
        'Successfully updated subscription with phased pricing plan',
      );
    } catch (error) {
  ...
  private async createInitialSubscriptionRecord(
    ...
    try {
      ...
      if (discountTier) {
        ...
        await supabase.from('attorney_subscriptions').insert({
  ...
  private async checkExistingSubscriptions(
    customerId: string,
  ): Promise<boolean> {
    try {
      this.logger.debug(
        `Checking existing subscriptions for customer: ${customerId}`,
      );
  ...
  private async handleSubscriptionPhaseTransition(
    subscription: Stripe.Subscription,
  ): Promise<void> {
    try {
      ...
      if (
        !subscription.metadata?.scheduled_phases ||
        !subscription.metadata?.current_phase
      ) {
        return;
      }
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          remainingDiscountMonths: Math.max(
            0,
            (currentSubscriptionData?.remainingDiscountMonths || 0) - 1,
          ),
          discountPercent:
            currentPhaseIndex === 0
              ? currentSubscriptionData?.discountPercent
              : 0,
        })
        .eq('stripesubscriptionid', subscription.id);
  ...
  private async handleSubscriptionUpdated(
    ...
    try {
      ...
      const { data: subscriptionData } = await supabase
        .from('attorney_subscriptions')
        .select(
          'remainingDiscountMonths, currentPeriodEnd, discountPercent, subscriptionStatus',
        )
        .eq('stripesubscriptionid', subscription.id)
        .single();
      ...
      const lastPeriodEnd = subscriptionData?.currentPeriodEnd
        ? new Date(subscriptionData.currentPeriodEnd).getTime()
        : 0;
      ...
      if (lastPeriodEnd === newPeriodEnd) {
        this.logger.debug(
          'Not a new billing period, skipping iteration update',
        );
        return;
      }
      ...
      const phases = JSON.parse(subscription.metadata.scheduled_phases);
      const currentPhaseIndex = parseInt(subscription.metadata.current_phase);
      const currentIterations = parseInt(
        subscription.metadata.current_iterations || '0',
      );
      ...
      if (currentPhase?.iterations && subscription.status === 'active') {
        ...
        if (subscriptionData?.remainingDiscountMonths > 0) {
          const newRemainingMonths =
            subscriptionData.remainingDiscountMonths - 1;
          ...
          const { error } = await supabase
            ...
            .update({
              ...
              discountPercent:
                newRemainingMonths > 0 ? subscriptionData.discountPercent : 0,
            })
            .eq('stripesubscriptionid', subscription.id);
  ...
  async getSession(sessionId: string): Promise<object> {
    try {
      ...
      this.logger.debug(
        'Successfully retrieved session and subscription details',
      );
